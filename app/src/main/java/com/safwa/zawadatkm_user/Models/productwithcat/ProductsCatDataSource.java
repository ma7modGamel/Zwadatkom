...
public class ProductsCatDataSource extends PageKeyedDataSource<Long, Datum> {
    public static int PAGE_SIZE = 8;
    public static long FIRST_PAGE = 1;

    String id;
    GlobalPrefrencies globalPrefrencies;
    public ProductsCatDataSource(Context context ,String id) {
        this.id=id;
    ...
    public void loadInitial(@NonNull LoadInitialParams<Long> params, @NonNull final LoadInitialCallback<Long, Datum> callback) {
        ...
        RetroWeb.getClient().create(ServiceApi.class).onGetAllProductByCat(id,FIRST_PAGE,globalPrefrencies.getLanguage(),"Bearer " +globalPrefrencies.getApi_token()).enqueue(new Callback<ProductsModel>() {
            @Override
    ...
    public void loadBefore(@NonNull final LoadParams<Long> params, @NonNull final LoadCallback<Long, Datum> callback) {
        ...
        RetroWeb.getClient().create(ServiceApi.class).onGetAllProductByCat(id,params.key,globalPrefrencies.getLanguage(),"Bearer " +globalPrefrencies.getApi_token()).enqueue(new Callback<ProductsModel>() {
            @Override
            public void onResponse(Call<ProductsModel> call, Response<ProductsModel> response) {
                ProductsModel body = response.body();
                if (body != null) {
